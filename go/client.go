// Package tordetect provides a Go client for the Tor Node Detector API.
//
// For more information, visit: https://apiverve.com/marketplace/tordetect?utm_source=go&utm_medium=readme
package tordetect
...
const (
	baseURL        = "https://api.apiverve.com/v1/tordetect"
	defaultTimeout = 30 * time.Second
)

// Client is the Tor Node Detector API client.
type Client struct {
	apiKey     string
	httpClient *resty.Client
}

// NewClient creates a new Tor Node Detector API client.
...
// Execute makes a request to the Tor Node Detector API with typed parameters.
//
...
//   - ip (required): string - The IP address to check against the Tor exit node list [format: ip]
